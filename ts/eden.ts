...
class Eden {
  ...
  private ready() {
    ...
    this._player = isEntity(_eval(this._world, [$('player-make'), chunk]));
    this.showChunk(chunk);

    let invChunk = _eval(this._world, [[$('Player'), $$('contents')], this._player]) as Chunk;
  ...
  private createChunk(): Chunk {
    ...
    _eval(this._world, [[$('Stairs'), $$('make')], chunk0, 1, 5, chunk1, 0, 5, false]);
    _eval(this._world, [[$('Stairs'), $$('make')], chunk1, 1, 5, chunk0, 2, 5, true]);
    return chunk0;
  }
  ...
  private keyDown(evt: KeyboardEvent) {
    switch (evt.keyCode) {
      ...
      // Go.
      case Key.G: _eval(this._world, [[$('Player'), $$('follow')], this._player]); break;

      // Create.
      case Key.C:
        _eval(this._world, [[[$('Player'), $$('add-inv')], this._player, [[$('Key'), $$('make')]]]]);
        break;
      case Key.V:
        _eval(this._world, [[[$('Player'), $$('add-inv')], this._player, [[$('Crate'), $$('make')]]]]);
        break;

      // Take, put.
      case Key.T: _eval(this._world, [[$('Player'), $$('take')], this._player]); break;
      case Key.P: _eval(this._world, [[$('Player'), $$('put')], this._player]); break;
  ...
  private move(dx: number, dy: number) {
    _eval(this._world, [[$('Player'), $$('move')], this._player, dx, dy]);
  }

  private select(slot: number) {
    _eval(this._world, [[$('Player'), $$('select')], this._player, slot]);
  }
  ...
  private tick() {
    ...
    let inv = _eval(this._world, [[$('Player'), $$('contents')], this._player]) as Chunk;
    inv.render(0, 0, 4);
